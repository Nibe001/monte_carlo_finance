...
- $\mathbb{E}(S_t) = \mathbb{E}(S_t | \mathcal{F}_0) = S_0 \exp\left((\mu - \frac12 \sigma^2)\right)\mathbb{E}(e^{\sigma \sqrt{t} W_1}) = S_0e^{\mu t}$ because $\mathbb{E}(e^{aX}) = e^{a^2 \over 2}$ when $X \sim \mathcal{N}(0, 1)$.
...
Let $N \ge 1$. Then for $N$ simulated i.i.d trajectories $(S^{(i)})_{i=1}^N$ until the maturity date $T$, we have the following estimator :
